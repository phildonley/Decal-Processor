...
function getImageFiles(folder) {
  ...
  for (var i = 0; i < all.length; i++) {
    var f = all[i];
    if (f instanceof File && f.name.match(/\\.(jpg|jpeg|png|psd)$/i)) imgs.push(f);
  }
  log("Found " + imgs.length + " images");
  return imgs;
}

//— Drop shadow ActionDescriptor (unchanged) —
...
//— True 30° perspective skew using tan(30°)=0.577 —
function applyLeftPerspectiveSkew(layer) {
  ...
  var offset    = Math.tan(Math.PI/6) * halfWidth; // ~0.577
  ...
  quad.putObject(charIDToTypeID("Pnt "), pointDescriptor(x1 + offset,   y1 - offset));
  quad.putObject(charIDToTypeID("Pnt "), pointDescriptor(x2 + offset,   y1 + offset));
  quad.putObject(charIDToTypeID("Pnt "), pointDescriptor(x2 - offset,   y2 + offset));
  quad.putObject(charIDToTypeID("Pnt "), pointDescriptor(x1 - offset,   y2 - offset));
...
function run() {
  var src    = selectFolder("Select folder with decal images"),
      output = selectFolder("Where to save output");
  ...
  for (var i = 0; i < imgs.length; i++) {
    try {
      ...
      alert("❌ Failed on " + imgs[i].name + ":\n" + e.message);
    }
  }

  alert("✅ Finished! See “Standard” & “Rotated” folders.");
}
